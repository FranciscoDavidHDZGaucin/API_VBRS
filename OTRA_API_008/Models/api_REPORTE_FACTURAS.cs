    ...
    public class api_REPORTE_FACTURAS
    {
        ...
        public Newtonsoft.Json.Linq.JArray REPORTE()
        {
            ...
            try
            {
                ...
                {
                    CONECT.Open();
                    using (SqlCommand COMANDO = new SqlCommand(@"SELECT  T1.DocNum, T1.CardCode, T1.CardName, T1.DocDate, T1.DocTotal,T2.U_NAME, T3.ZONA, T3.COD_ZONA, T3.Division_Comercial FROM  [VM-SQL\SAP].AGROVERSA_PRODUCTIVA.dbo.OINV AS T1 INNER JOIN [VM-SQL\SAP].AGROVERSA_PRODUCTIVA.dbo.OUSR AS T2 ON T1.UserSign = T2.USERID INNER JOIN vwCAT_AGENTES AS T3 ON T3.COD_ZONA = T1.SlpCode WHERE YEAR(T1.DocDate)=2020 and T1.CANCELED = 'N'", CONECT))
