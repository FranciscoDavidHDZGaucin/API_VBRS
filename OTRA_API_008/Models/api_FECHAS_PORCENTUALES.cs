...
{
    public class api_FECHAS_PORCENTUALES
    {
        private SqlDataReader drd;

        public List<DataTable> _RES_TOTALES = null;

        public Newtonsoft.Json.Linq.JArray FECHAS_PORCEN()
        {
            ...
            try
            {
                ...
                {
                    CONECT.Open();
                    using (SqlCommand COMANDO = new SqlCommand("SELECT  FECHA_CTRL,mes, SEMANARSGD ,Meta FROM TB_GOPC_FECHAS_PORCENTUALES", CONECT))
