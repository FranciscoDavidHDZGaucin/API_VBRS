...
{
    public class api_SAC_PEDIDOS_ELIMINADOS
    {
        private SqlDataReader drd;

        public List<DataTable> _RES_TOTALES = null;

        public Newtonsoft.Json.Linq.JArray PEDIDOS_BORRADOS()
        {
            ...
            try
            {
                ...
                {
                    CONECT.Open();
                    using (SqlCommand COMANDO = new SqlCommand("SELECT * FROM VW_PEDIDOS_BORRADOS", CONECT))
